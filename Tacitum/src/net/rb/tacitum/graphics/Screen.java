...
import net.rb.tacitum.entity.mob.Chaser;
import net.rb.tacitum.entity.mob.Dummy;
import net.rb.tacitum.entity.mob.Mob;
import net.rb.tacitum.entity.mob.Shooter;
import net.rb.tacitum.entity.mob.Star;
	...
	public void renderTextCharacter(int xp, int yp, Sprite sprite, int color, boolean fixed) {
		...
		for (int y = 0; y < sprite.getHeight(); y++) {
			...
			for (int x = 0; x < sprite.getWidth(); x++) {
				...
				if (col != ALPHA_COLOR && col != 0xFF7F007F) pixels[xa + ya * width] = color;
			}
		}
	}
	...
	public void renderSprite(int xp, int yp, Sprite sprite, boolean fixed) {
		...
		for (int y = 0; y < sprite.getHeight(); y++) {
			...
			for (int x = 0; x < sprite.getWidth(); x++) {
				...
				if (xa < 0 || xa >= width || ya < 0 || ya >= height) continue;
				int col = sprite.pixels[x + y * sprite.getWidth()];
				if (col != ALPHA_COLOR && col != 0xFF7F007F) pixels[xa + ya * width] = col;
			}
		}
	}
		...
		for (int y = 0; y < mob.getSprite().getHeight(); y++) {
			...
			for (int x = 0; x < mob.getSprite().getWidth(); x++) {
				...
				if (mob instanceof Chaser || mob instanceof Star) {
					...
					if (col == 0xFFAC6C1F) col = 0x40072A;
					if (col == 0xFFFEFF60) col = 0x610A41;  // Cape & Crown inner
					if (col == 0xFFDEE03F) col = 0x8A135E;  // Cape & Crown outer
					if (col == 0xFFD81C5A) col = 0xFF5E5E;  // Necklace
					if (col == 0xFF472BBF) col = 0xD93232;  // Armor inner
					if (col == 0xFF9787DD) col = 0xA62121;  // Armor outer
				}
				
				if (mob instanceof Dummy || mob instanceof Shooter) {
					if (col == 0xFFD81C5A) col = 0xB8F5B9;
					if (col == 0xFF472BBF) col = 0x529EB3;
					if (col == 0xFF9787DD) col = 0xB8F5F1;
				}
